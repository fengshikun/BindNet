...
run_name="DUDE${fold}_crnet_recycle${recycling}_lr${lr}_bz${batch_size}_crnetFreeze${freeze_pretrained_transformer}_seed${seed}"
save_dir=./results/DUDE${fold}_crnet_recycle${recycling}_lr${lr}_bz${batch_size}_crnetFreeze${freeze_pretrained_transformer}_seed${seed}
...
CUDA_VISIBLE_DEVICES=$gpu_use python -m torch.distributed.launch --nproc_per_node=$n_gpu --master_port=$MASTER_PORT $(which unicore-train) .  \
       ...
       --keep-best-checkpoints 1 --best-checkpoint-metric test_RE_05pa --maximize-best-checkpoint-metric \
       --no-last-checkpoints \
       --patience 10 \
       --freeze-pretrained-transformer ${freeze_pretrained_transformer} \
       --tmp-save-dir ./tmp/DUDE${fold}/ \
       --DUDE_fold $fold \
       --DUDE_data $data \
       --fp16 \
       --all-test 1 \
