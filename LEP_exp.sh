...
CUDA_VISIBLE_DEVICES=0 python -m torch.distributed.launch --nproc_per_node=$n_gpu --master_port=$MASTER_PORT $(which unicore-train) .  \
       ...
       --task LEP --loss finetune_cross_entropy --arch LEPClassification  \
       --task-name LEP \
       --optimizer adam --adam-betas "(0.9, 0.99)" --adam-eps 1e-6 --clip-norm 1.0 --weight-decay $wd \
       --lr-scheduler polynomial_decay --lr $lr --warmup-updates $warmup_steps --total-num-update $max_steps \
       --update-freq $update_freq --seed $seed \
       --tensorboard-logdir $save_dir/tsb \
       --max-update $max_steps --log-interval 5 --log-format simple \
       --save-interval-updates 10 --validate-interval-updates 10 --keep-interval-updates 1 --no-epoch-checkpoints  \
       ...
       --keep-best-checkpoints 1 --best-checkpoint-metric valid_auc --maximize-best-checkpoint-metric \
       --LEP_data $LEP_data \
       --no-last-checkpoints \
       --patience 5 \
       --freeze-pretrained-transformer ${freeze_pretrained_transformer} \
       --tmp-save-dir ./tmp/LEP_tmp/ \
       --use-BindNet 1 \
