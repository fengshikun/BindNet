...
@register_model("LEPClassification")
class LEPClassificationModel(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
    ...
    def __init__(self, args, dictionary, lig_dictionary=None):
        ...
        if self.args.CLS_use == "complex_CLS":
            ...
            self.classification_head = NonLinearHead(args.encoder_embed_dim * 4, args.target_class, args.activation_fn)
        else:
    ...
    def forward(
        ...
        src_edge_type,
        inactive_src_tokens,
        inactive_src_distance,
        inactive_src_edge_type,
        inactive_lig_tokens,
        inactive_lig_distance,
        inactive_lig_edge_type,
        ...
        **kwargs
        ...
        '''inactive Part'''
        # Protein Encoder Calculation
        prot_padding_mask = inactive_src_tokens.eq(self.padding_idx)    # Recond the padding token
        if self.args.proc_freeze:
            with torch.no_grad():
                x = self.embed_tokens(inactive_src_tokens)
                graph_attn_bias = get_dist_features(inactive_src_distance, inactive_src_edge_type)
                (
                    encoder_rep,
                    encoder_pair_rep,
                    delta_encoder_pair_rep,
        ...
        # Ligand Encoder Calculation
        lig_padding_mask = inactive_lig_tokens.eq(self.padding_idx)
        if self.args.ligand_freeze:
            with torch.no_grad():
                lig_x = self.lig_embed_tokens(inactive_lig_tokens)
                lig_graph_attn_bias = get_dist_feature_lig(inactive_lig_distance, inactive_lig_edge_type)
                (
                    lig_encoder_rep,
                    lig_encoder_pair_rep,
                    lig_delta_encoder_pair_rep,
        ...
        if self.args.net == 'complex_crnet': # like unimol:
            ...
            inactive_mol_decoder = decoder_rep[:, :mol_sz]
            inactive_pocket_decoder = decoder_rep[:, mol_sz:]



        '''Affinity Regression Head Calculation Part'''

        protein_cls_token = pocket_decoder[:, 0, :]
        ligand_cls_token = mol_decoder[:, 0, :]
        inactive_protein_cls_token = inactive_pocket_decoder[:, 0, :]
        inactive_ligand_cls_token = inactive_mol_decoder[:, 0, :]

        cls_token = torch.cat([protein_cls_token, ligand_cls_token, inactive_protein_cls_token, inactive_ligand_cls_token], dim=-1)
...
@register_model_architecture("LEPClassification", "LEPClassification")
