...
task_metainfo = {
    'LBA_60_affinity' : {
        'mean': 6.479,
        'std': 1.970,
        'target_name': 'LBA_60_affinity'
    },
}



@register_task("LBA_60")
class LBA_60(UnicoreTask):
    @staticmethod
    def add_args(parser):
    ...
    def __init__(self, args, dictionary, lig_dictionary):
        ...
        self.LBA_random_data = self.args.LBA_data
    ...
    def load_dataset(self, split, combine=False, **kwargs):
        ...
        """
        raw_dataset = make_LBA60_dataset(data=self.LBA_random_data, idx_path=self.idx_path, max_num=self.max_comnum, split=split)

        def one_dataset(raw_dataset, coord_seed, mask_seed):
            '''Get the input and target of one raw_dataset'''
            # if self.args.mode =='train':
            dataset = ExtractLBA60Dataset(raw_dataset, "smi", "atoms", "coordinates", task='aff_finetune')
            smi_dataset = KeyDataset(dataset, "smi")
            if hasattr(self.args, 'use_frad') and self.args.use_frad:
                frad_dataset = FradDataset_LBA60(dataset)
